...
import { currentParticipant, setParticipant } from '../actions/workshop';
    ...
    <Wrapper>
      ...
      {!isObjectEmpty(currentWorkshop) && (
        <>
          ...
          {/* Else: there is no currentParticipant, render all verified users */}
          {isObjectEmpty(currentParticipant) && (
            <>
              <h2 className='text-align'>רשימת הממתינים לאישור</h2>
              <div className='participants-container'>
                {currentWorkshop.pending.map((item) => {
                  if (item.verified) {
                    return (
