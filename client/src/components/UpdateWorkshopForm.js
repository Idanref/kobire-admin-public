...
import { updateWorkshop } from '../actions/workshop';

const UpdateWorkshopForm = ({ updateWorkshop, currentWorkshop }) => {
  ...
  // update current workshop details in form
  useEffect(() => {
    const date = new Date(currentWorkshop.date);
    setLocation(currentWorkshop.location);
    ...
    setPrice(currentWorkshop.price);
    setPayBoxUrl(currentWorkshop.payBoxUrl);
    setBitUrl(currentWorkshop.bitUrl);
    setMaxSpots(currentWorkshop.maxSpots);
    setTakenSpots(currentWorkshop.takenSpots);
  }, [currentWorkshop]);
  ...
  const handleSubmit = (e) => {
    ...
    if (parseInt(takenSpots) < 0 || parseInt(maxSpots) < 0) {
      return alert('מספר משתתפים לא יכול להיות שלילי');
    }
    ...
    const data = {
      ...
      takenSpots: parseInt(takenSpots),
      bitUrl,
      payBoxUrl,
      price: parseInt(price),
    };

    updateWorkshop(data, currentWorkshop._id);
  };
...
UpdateWorkshopForm.propTypes = {
  updateWorkshop: PropTypes.func.isRequired,
...
export default connect(mapStateToProps, { updateWorkshop })(UpdateWorkshopForm);
